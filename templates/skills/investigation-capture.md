...
To confirm the metadata actually landed on the doc, query the slot by the fields you DID set (do NOT filter by `atom_type` — investigations don't carry one; the canonical atom_type set is the eight flush/compile types plus `plan`, none of which is "investigation"). For example `search_memory({ query: "<investigation title>", datasets: ["investigations"], filters: { task_type: "debugging", project_module: "<module>" } })` returns the doc when its metadata wrote correctly and returns nothing (or an unfiltered hit) when a metadata write failed. The `audit_memory` tool's `missing-metadata` class does NOT inspect the `investigations` slot (it walks `knowledge` and `self_improvement`), so this manual `search_memory` check is the verification path for investigations. Separately, `audit_memory({classes: ["stale-plans"]})` is a sanity check for the sibling `plans` slot; there's no investigations-specific audit class because investigations don't suffer the same title-drift issue plans do (you, the agent, picked the slug deliberately).
