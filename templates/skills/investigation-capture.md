...
Run `audit_memory({classes: ["missing-metadata"]})` after a save to confirm `project_module` / `task_type` / `error_pattern` actually landed on the doc; the audit walks `knowledge` and `self_improvement` for missing required fields (it does NOT currently inspect the `investigations` slot, but the same check via `search_memory({filters: {atom_type: "investigation", ...}, datasets: ["investigations"]})` reveals any metadata-write failures). And `audit_memory({classes: ["stale-plans"]})` is a related sanity check for the sibling `plans` slot; there's no investigations-specific audit class because investigations don't suffer the same title-drift issue plans do (you, the agent, picked the slug deliberately).
