...
The audit_memory MCP tool walks the `plans` slot and surfaces any doc whose slug is a hyphen-delimited prefix of a newer doc's slug (so `plan-auth` is flagged when `plan-auth-rewrite` exists, but NOT when only `plan-oauth` or `plan-authz` exists — those aren't hyphen-delimited extensions). Each finding carries `documentId`, `name`, `reason`, and `suggested_action` (`delete` for plain rename leftovers). Then act on each finding with the appropriate tool:
