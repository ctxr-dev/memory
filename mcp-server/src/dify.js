...
// injection when a user pastes a key with stray newline into ./.memory/settings/.env.
...
export function requireDifyWriteConfig(config, datasetNameOrId) {
  ...
  if (datasetNameOrId) {
    ...
    if (!resolved) {
      throw new Error(
        `Dataset '${datasetNameOrId}' is not configured. Add ${slotEnvKey(datasetNameOrId)}=<dataset-id> to ./.memory/settings/.env (every DIFY_DATASET_<NAME>_ID line declares one slot), or run ./memory/scripts/dify-setup.sh.`,
      );
    }
    return resolved;
  }
  ...
  if (!fallback) {
    throw new Error(
      "No write dataset configured. Run ./memory/scripts/dify-setup.sh, or add a DIFY_DATASET_<NAME>_ID=<dataset-id> line to ./.memory/settings/.env.",
    );
  }
  return fallback;
}
