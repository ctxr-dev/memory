...
if ! docker inspect -f '{{.State.Running}}' "$container_name" >/dev/null 2>&1; then
  echo "MCP smoke failed: $container_name is not running. Start the stack with up.sh first." >&2
  exit 1
fi
...
if (!Array.isArray(config.datasetIds) || config.datasetIds.length === 0) {
  fail("No datasets configured. Run dify-setup.sh to bind dataset slots (every DIFY_DATASET_<NAME>_ID line declares one).");
}
...
if (!flushBound && !config.legacyWriteDatasetId) {
  fail(`Flush slot '${flushSlot}' has no configured id. Run dify-setup.sh.`);
}
if (!compileBound && !config.legacyWriteDatasetId) {
  fail(`Compile slot '${compileSlot}' has no configured id. Run dify-setup.sh.`);
}
let warnings = 0;
if (!flushBound && config.legacyWriteDatasetId) {
  warnings += 1;
  console.error(`MCP smoke WARNING: flush slot '${flushSlot}' is unbound; writes fall back to DIFY_WRITE_DATASET_ID=${config.legacyWriteDatasetId}. Run dify-setup.sh to bind the slot.`);
}
if (!compileBound && config.legacyWriteDatasetId) {
  warnings += 1;
  console.error(`MCP smoke WARNING: compile slot '${compileSlot}' is unbound; writes fall back to DIFY_WRITE_DATASET_ID=${config.legacyWriteDatasetId}. Run dify-setup.sh to bind the slot.`);
}
