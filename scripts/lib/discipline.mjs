// Host-side copy of the memory discipline. The Dockerised MCP bridge cannot
// import scripts/lib/ (mcp-server/Dockerfile copies only mcp-server/src), so the
// INSTRUCTIONS text is duplicated in mcp-server/src/discipline.js and the two
// copies are locked byte-identical by test/discipline.test.mjs. This host copy
// adds buildSessionStartContext, used by the Claude Code SessionStart hook.
// No em or en dashes (project rule).
...
// Longer SessionStart context: names the server, embeds the discipline, adds the
// Dify-slot + compile detail, and a compile-trigger note.
export function buildSessionStartContext({ serverName = "<memory-server>", compileTriggered = false } = {}) {
  return [
    `Project memory is available through the \`${serverName}\` MCP server.`,
    INSTRUCTIONS,
