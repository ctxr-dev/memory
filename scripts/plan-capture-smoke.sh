...
if ! docker inspect -f '{{.State.Running}}' "$container_name" >/dev/null 2>&1; then
  echo "plan-capture-smoke SKIP: $container_name is not running. Start it with up.sh." >&2
  exit 0
fi

plans_id="${DIFY_DATASET_PLANS_ID:-$(read_env_value DIFY_DATASET_PLANS_ID "$MEMORY_ENV" 2>/dev/null || true)}"
if [ -z "$plans_id" ]; then
  echo "plan-capture-smoke SKIP: DIFY_DATASET_PLANS_ID is empty. Run dify-setup.sh to bind the plans slot." >&2
  exit 0
fi
...
if [ -z "$dify_key" ]; then
  echo "plan-capture-smoke SKIP: DIFY_KNOWLEDGE_API_KEY is empty in $MEMORY_ENV. Run dify-setup.sh to configure Dify access." >&2
  exit 0
fi
