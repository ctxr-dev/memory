...
- Test suite grew from 156 to 250 tests (5 new test files plus extensions to several existing files). New: `test/exit-plan-mode.test.mjs` (planDocSpec contract), `test/exit-plan-mode-cli.test.mjs` (CLI driver always-exit-0 invariants), `test/dify-write.test.mjs` (saveDocument flag-builder), `test/env.test.mjs` (slotEnvKey + envValue precedence), `test/lib-helpers.test.mjs` (shared test-helper contract). Extended: `test/datasets.test.mjs` (schema parity bridge↔host + type lock + ATOM_TYPES cross-runtime parity), `test/redact.test.mjs` (DB / Azure / npm / Anthropic patterns), `test/merge-config.test.mjs` (matcher-collision regression for the new PostToolUse entry), `test/cross-runtime-slug-sync.test.mjs` (slotEnvKey host↔bridge parity including non-string falsy inputs), `test/dify-pure.test.mjs` (enable/disable URL+body lock, pickDuplicatesToDelete null-guard, workspace.js defaults).
