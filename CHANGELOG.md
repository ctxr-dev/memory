...
- **`delete_document` and `disable_document` MCP tools.** Surface the bridge's existing delete/disable primitives so agents can clean up stale `plan-<slug>.md` (or any other doc) without dropping into the Dify UI. `delete_document` is permanent; `disable_document` is reversible (hides from search, keeps for audit).
...
- Tests went from 156 (pre-feature) → 230 (after round-24) → 232+ (after round-25). New test files: `test/exit-plan-mode.test.mjs`, `test/exit-plan-mode-cli.test.mjs`, `test/dify-write.test.mjs`, `test/env.test.mjs`. Existing files extended: `test/datasets.test.mjs` (schema parity), `test/redact.test.mjs` (new patterns), `test/merge-config.test.mjs` (matcher-collision regression for the new PostToolUse entry).
